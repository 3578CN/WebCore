...
namespace CBC.WebCore.WebView.DataControl
{
    ...
    public class PagerControl : Control
    {
        ...
        private readonly string _pagerUrl; // 分页 URL。
        ...
        public PagerControl(HttpContext context, int totalRowCount, int pageSize, int currentPage)
            : this(context, null, pageSize, currentPage)
        {
        ...
        /// <param name="pageSize">每页行数。</param>
        /// <param name="currentPage">当前页码。</param>
        public PagerControl(HttpContext context, DataTable dataTable, int pageSize, int currentPage)
        {
            ...
            // 移除已有的 Page 参数。
            var queryParams = queryString.Where(p => !string.Equals(p.Key, "Page", StringComparison.OrdinalIgnoreCase))
                                         .ToDictionary(p => p.Key, p => p.Value.ToString());

            if (queryParams.Count == 0)
            {
                // 如果没有其他参数，直接添加 Page 参数。
                _pagerUrl = $"{request.Path}?Page=";
            }
            else
            {
                // 有其他参数时，构建查询字符串并追加 Page 参数。
                _pagerUrl = $"{request.Path}?{string.Join("&", queryParams.Select(kvp => $"{kvp.Key}={kvp.Value}"))}&Page=";
            }
        ...
        protected override async Task ProcessTemplateCodeAsync()
        {
            ...
            var templateContent = await ResourceProcessor.LoadTemplateAsync(PagerTemplate, true);
        ...
        private string GetPageLink(JToken pagerTemplate, bool isValid, string validKey, string invalidKey, int pageNumber)
        {
            ...
            return isValid ? ReplaceTag(pageLink, "PageUrl", $"{_pagerUrl}{pageNumber}") : pageLink;
        }
        ...
        private string GetNumberPaginate(JToken pagerTemplate)
        {
            ...
            for (var i = startPage; i <= endPage; i++)
            {
                ...
                else
                {
                    ...
                    pageLink = ReplaceTag(pagerTemplate["valid_NumberPaginate"]?.ToString(), "PageUrl", $"{_pagerUrl}{i}");
                }
        ...
        private string GetDropdownPaginate(JToken pagerTemplate)
        {
            ...
            for (var i = startPage; i <= endPage; i++)
            {
                ...
                {
                    // 其他页使用未选中的下拉列表模板。
                    pageLink = ReplaceTag(pagerTemplate["unselected_DropdownPaginate"]?.ToString(), "PageUrl", $"{_pagerUrl}{i}");
                }
